...
<form method="POST" action="{{ route('admin.categories.update', $category) }}" class="max-w-xl space-y-6">
    ...
    <div>
        ...
        <textarea name="description" rows="4"
            class="w-full bg-slate-800 border border-slate-700 rounded p-2 focus:outline-none focus:ring focus:ring-emerald-600">{{ old('description', $category->description) }}</textarea>
    </div>
